...
export const dealTaskPublicResource = dealTaskResource.omit({ workspaceId: true })
...
export const dealTaskIdParams = dealIdParam.extend({
  taskId: zodBigintAsString().describe("Task id. Get it from `deals.listTasks`."),
})
...
export const stageTemplateParams = z.object({
  id: zodBigintAsString().describe("Pipeline id. Get it from `pipelines.list`."),
