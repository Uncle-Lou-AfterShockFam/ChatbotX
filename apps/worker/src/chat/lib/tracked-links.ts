import { buildTrackedLinkUrl, trackedLinkService } from "@chatbotx.io/business"
 ...
 * the pull outbox and the stored Message all carry the short URLs. Any
 * other step comes back untouched. A mint failure throws: the operator
 * opted in, so sending the raw link silently would defeat the option.
 */
...
>(input: {
  ...
  const { step } = input
  if (
    step.stepType !== stepTypes.enum.bulktextSend ||
    !("trackLinks" in step) ||
  ...
  const { text } = await rewriteTrackedLinks(step.text, input.appUrl, (url) =>
    trackedLinkService.mint({
