...
describe("documents html", () => {
  ...
  test("a value can never plant a Documenso field; control chars and oversize are refused", () => {
    for (const bad of [
      ...
      expect(() => mergeDocumentHtml(BODY, { first_name: bad })).toThrow(DocumentMergeError)
    }
    expect(mergeDocumentHtml(BODY, { first_name: "x".repeat(MAX_MERGE_VALUE_LENGTH) })).toContain("x".repeat(100))
  })

  test("newlines in a value become <br>; prototype keys are not merge values", () => {
    expect(mergeDocumentHtml("<p>{{addr}}</p>", { addr: "1 Main\nApt 2" })).toBe("<p>1 Main<br>Apt 2</p>")
    expect(mergeDocumentHtml("<p>{{constructor}} {{__proto__}}</p>", {})).toBe("<p>{{constructor}} {{__proto__}}</p>")
  })
  ...
  test("fuzz: a merged value is escaped or refused, never raw markup or braces", () => {
    ...
    let merged = 0
    for (let i = 0; i < 500; i++) {
      const v = Array.from({ length: Math.floor(Math.random() * 10) }, () => alphabet[Math.floor(Math.random() * alphabet.length)]).join("")
      let out: string
      try {
      ...
      expect(inner.replace(/<br>/g, "")).not.toMatch(/[<>"']/)
